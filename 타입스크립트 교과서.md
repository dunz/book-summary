...
# 스니펫
## `infer`를 이용해 정렬을 역으로 돌리는 Reverse 만들기
```ts
...
## 템플릿 리터럴 타입과 `infer`를 이용해서 공백을 지우는 타입 만들기
```ts
...
```
기본적으로 `string` 과 `number` 는 겹치지 않지만 `number & any` 가 되면 `any` 가 되고 `string` 은 `any` 를 extends 할 수 있게 된다
...
type IsArray<T> = IsNever<T> extends true 
    ...
    : T extends readonly unknown[]
        ...
        : false
```
- `IsArray<never>`가 `never`가 되는 것을 막기 위해 `IsNever<T> extends true`가 필요
- `IsArray<any>`가 `boolean` 이 되는 것을 막기 위해 `IsAny<T> extends true`가 필요
...
type IsTuple<T> = IsNever<T> extends true 
    ...
    : T extends readonly unknown[]
        ...
        : false
```
- 배열과 튜플의 가장 큰 차이는 튜플은 길이가 고정되어 있다는 것
- 배열의 `length는` `number`이고 튜플은 1,2,3 처럼 리터럴 숫자이다
...
type IsUnion<T, U = T> = IsNever<T> extends true 
    ...
    : T extends T
        ...
        : false
```
- 유니온 타입이 컨디셔널 타입을 만나면 분배법칙이 발생한다, 따라서 `T extends T`는 유니온의 분배법칙을 발새시키기 위함
- 즉 `T`가 `string | number`인 경우 `T extends T`는 `string | number extends string | number`가 아니라  
  `string extends string | number` | `number extends string | number`가 된다
- `U`에는 분배법칙이 발생하지 않도록 `T`원본을 담아두었기 때문에 `[U] extends [T]`는   
  `[string | number] extends [string]` 또는 `[string | number] extends [number]` 가 된다

## 차집합
```ts
type Diff<A, B> = Omit<A & B, keyof B>
```
- `Omit`은 특정 객체에서 지정한 속성을 제거하는 타입
